...
vec2 sdfMap (vec3 position){
    ...
    vec3 planePosition = (position - vec3(0.,-110.1,0.));
...
vec3 calcNormalZMap (in Ray ray){
    float eps = 0.001 * ray.length;   // replace by an appropriate value
    vec2 h = vec2(eps,0);
...
vec3 GetColor(Ray ray, float surfaceId, vec2 uv){
    ...
    if (surfaceId == ID_GROUND) {
        ...
        vec3 color2 = 1.0*.3 * calcLightning(ray, lightDirection2, vec3(0.3176, 0.2549, 0.0235), surfaceId);
        color = color1 + color2;
        }
