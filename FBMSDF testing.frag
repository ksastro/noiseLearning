...
const int NOISE_ITERATION_LIMIT = 40;
...
vec2 map (vec3 position, float pixelWidth){
    ...
    //float d = sdSphere(position, vec3(-0.3,0,.1*u_time),0.4);
    //result = vec2(opSmoothIntersection(d, sdSphere(position, vec3(0.3,0,0.1*u_time),0.4), 0.1),ID_TERRAIN);
...
vec3 calcNormalCentralDifference(in vec3 position, in float pixelWidth)
{
    vec2 h = vec2(0.1 * pixelWidth,0);
    return normalize( vec3(map(position+h.xyy, pixelWidth).x - map(position-h.xyy, pixelWidth).x,
                           map(position+h.yxy, pixelWidth).x - map(position-h.yxy, pixelWidth).x,
                           map(position+h.yyx, pixelWidth).x - map(position-h.yyx, pixelWidth).x ) );
}
vec3 getNormal(in Ray ray){
    vec3 normal = calcNormalTetrahedron(ray.position, ray.footprint);
